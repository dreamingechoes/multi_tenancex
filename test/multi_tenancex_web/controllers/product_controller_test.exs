defmodule MultiTenancexWeb.ProductControllerTest do
  ...
  @create_attrs %{description: "some description", image: "some image", name: "some name", price: 120.5, units: 42}
  ...
  describe "create product" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post conn, admin_product_path(conn, :create), product: @create_attrs
  ...
  describe "update product" do
    ...
    test "redirects when data is valid", %{conn: conn, product: product} do
      conn = put conn, admin_product_path(conn, :update, product), product: @update_attrs
